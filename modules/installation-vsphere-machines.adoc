...
**** Note: For the bootstrap node, ignition must be provided in `guestinfo.ignition.config.data` in the *Configuration Parameters* window.  This is due to a restriction in the maximum size of data which may be provided in a vApp property.
